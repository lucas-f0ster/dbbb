export default class MergedInput extends Phaser.Plugins.ScenePlugin {
  ...
  pointerMove (pointer, threshold) {
    ...
    if (pointer.distance > threshold) {
      const pointerDirection = this.getBearingFromAngle(pointer.angle, 8)
      ...
      if (typeof this.players[0] !== 'undefined' && this.players[0].position.x !== 'undefined') {
        ...
        const angleToPointer = Phaser.Math.Angle.Between(position.x, position.y, pointer.worldX, pointer.worldY)
        const pointerDirection = this.getBearingFromAngle(angleToPointer, 8)
        const pointerAngle = Number(this.mapBearingToDegrees(pointerDirection))
